...
module Make(T : Sig.INET_TYPE) = struct
  type port = AuxPort of int | MainPort
  type label = {src_port : port; dst_port : port; l : T.label}
  ...
  let premises net v =
    ...
    let filter_pred e l = match E.label e with
      ...
      | _ -> l in
    fold_succ_e filter_succ net v []
    |> fold_pred_e filter_pred net v 

  let conclusion net v =
    let finder e = match E.label e with
        {src_port=MainPort} when E.src e=v -> true
      | {dst_port=MainPort} when E.dst e=v -> true
      | _ -> false
    in
    let iterator f n =
      iter_pred_e f n v;
      iter_succ_e f n v
    in find_edge_p_ iterator finder net
 
  let net_conclusions net = (premises net end_vertex)

  let merge net net' =
    let v_adder = function
      | (_,t) when t=T.end_type -> ()
...
(*
module MELLInteractionNet : Sig.REWRITABLE_INET = struct
  module MELLINetType : Sig.INET_TYPE = struct
    type vertex_type = [
        `End
      | `One
      | `Bot
